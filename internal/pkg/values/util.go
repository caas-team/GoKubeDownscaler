...
type logEventFunc func(reason, id, message string, ctx context.Context)

// GetLayerFromAnnotations makes a layer and fills it with all values from the annotations
func GetLayerFromAnnotations(annotations map[string]string, logEvent logEventFunc, ctx context.Context) (Layer, error) {
	result := NewLayer()
	var err error
	...
	if downscalePeriod, ok := annotations[annotationDownscalePeriod]; ok {
		...
		if err != nil {
			logEvent(reasonInvalidAnnotation, annotationDownscalePeriod, fmt.Sprintf("failed to parse %q annotaion: %s", annotationDownscalePeriod, err.Error()), ctx)
	...
	if downtime, ok := annotations[annotationDowntime]; ok {
		...
		if err != nil {
			logEvent(reasonInvalidAnnotation, annotationDowntime, fmt.Sprintf("failed to parse %q annotaion: %s", annotationDowntime, err.Error()), ctx)
	...
	if upscalePeriod, ok := annotations[annotationUpscalePeriod]; ok {
		...
		if err != nil {
			logEvent(reasonInvalidAnnotation, annotationUpscalePeriod, fmt.Sprintf("failed to parse %q annotaion: %s", annotationUpscalePeriod, err.Error()), ctx)
	...
	if uptime, ok := annotations[annotationUptime]; ok {
		...
		if err != nil {
			logEvent(reasonInvalidAnnotation, annotationUptime, fmt.Sprintf("failed to parse %q annotaion: %s", annotationUptime, err.Error()), ctx)
	...
	if exclude, ok := annotations[annotationExclude]; ok {
		...
		if err != nil {
			logEvent(reasonInvalidAnnotation, annotationExclude, fmt.Sprintf("failed to parse %q annotaion: %s", annotationExclude, err.Error()), ctx)
	...
	if excludeUntil, ok := annotations[annotationExcludeUntil]; ok {
		...
		if err != nil {
			logEvent(reasonInvalidAnnotation, annotationExcludeUntil, fmt.Sprintf("failed to parse %q annotaion: %s", annotationExcludeUntil, err.Error()), ctx)
	...
	if forceUptime, ok := annotations[annotationForceUptime]; ok {
		...
		if err != nil {
			logEvent(reasonInvalidAnnotation, annotationForceUptime, fmt.Sprintf("failed to parse %q annotaion: %s", annotationDowntime, err.Error()), ctx)
	...
	if forceDowntime, ok := annotations[annotationForceDowntime]; ok {
		...
		if err != nil {
			logEvent(reasonInvalidAnnotation, annotationForceDowntime, fmt.Sprintf("failed to parse %q annotaion: %s", annotationForceDowntime, err.Error()), ctx)
	...
	if downscaleReplicas, ok := annotations[annotationDownscaleReplicas]; ok {
		...
		if err != nil {
			logEvent(reasonInvalidAnnotation, annotationDownscaleReplicas, fmt.Sprintf("failed to parse %q annotaion: %s", annotationDownscaleReplicas, err.Error()), ctx)
	...
	if gracePeriod, ok := annotations[annotationGracePeriod]; ok {
		...
		if err != nil {
			logEvent(reasonInvalidAnnotation, annotationGracePeriod, fmt.Sprintf("failed to parse %q annotaion: %s", annotationGracePeriod, err.Error()), ctx)
