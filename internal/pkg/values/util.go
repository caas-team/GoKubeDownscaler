...
const (
	...
	reasonIncompatible = "Incompatible Configuration"
)

type resourceLogger interface {
	// ErrorInvalidConfiguration adds an invalid configuration error on a resource
	ErrorInvalidConfiguration(id string, message string, ctx context.Context)
}
...
func GetLayerFromAnnotations(annotations map[string]string, logEvent resourceLogger, ctx context.Context) (Layer, error) {
	...
	if downscalePeriod, ok := annotations[annotationDownscalePeriod]; ok {
		...
		if err != nil {
			logEvent.ErrorInvalidConfiguration(annotationDownscalePeriod, fmt.Sprintf("failed to parse %q annotaion: %s", annotationDownscalePeriod, err.Error()), ctx)
	...
	if downtime, ok := annotations[annotationDowntime]; ok {
		...
		if err != nil {
			logEvent.ErrorInvalidConfiguration(annotationDowntime, fmt.Sprintf("failed to parse %q annotaion: %s", annotationDowntime, err.Error()), ctx)
	...
	if upscalePeriod, ok := annotations[annotationUpscalePeriod]; ok {
		...
		if err != nil {
			logEvent.ErrorInvalidConfiguration(annotationUpscalePeriod, fmt.Sprintf("failed to parse %q annotaion: %s", annotationUpscalePeriod, err.Error()), ctx)
	...
	if uptime, ok := annotations[annotationUptime]; ok {
		...
		if err != nil {
			logEvent.ErrorInvalidConfiguration(annotationUptime, fmt.Sprintf("failed to parse %q annotaion: %s", annotationUptime, err.Error()), ctx)
	...
	if exclude, ok := annotations[annotationExclude]; ok {
		...
		if err != nil {
			logEvent.ErrorInvalidConfiguration(annotationExclude, fmt.Sprintf("failed to parse %q annotaion: %s", annotationExclude, err.Error()), ctx)
	...
	if excludeUntil, ok := annotations[annotationExcludeUntil]; ok {
		...
		if err != nil {
			logEvent.ErrorInvalidConfiguration(annotationExcludeUntil, fmt.Sprintf("failed to parse %q annotaion: %s", annotationExcludeUntil, err.Error()), ctx)
	...
	if forceUptime, ok := annotations[annotationForceUptime]; ok {
		...
		if err != nil {
			logEvent.ErrorInvalidConfiguration(annotationForceUptime, fmt.Sprintf("failed to parse %q annotaion: %s", annotationForceUptime, err.Error()), ctx)
	...
	if forceDowntime, ok := annotations[annotationForceDowntime]; ok {
		...
		if err != nil {
			logEvent.ErrorInvalidConfiguration(annotationForceDowntime, fmt.Sprintf("failed to parse %q annotaion: %s", annotationForceDowntime, err.Error()), ctx)
	...
	if downscaleReplicas, ok := annotations[annotationDownscaleReplicas]; ok {
		...
		if err != nil {
			logEvent.ErrorInvalidConfiguration(annotationDownscaleReplicas, fmt.Sprintf("failed to parse %q annotaion: %s", annotationDownscaleReplicas, err.Error()), ctx)
	...
	if gracePeriod, ok := annotations[annotationGracePeriod]; ok {
		...
		if err != nil {
			logEvent.ErrorInvalidConfiguration(annotationGracePeriod, fmt.Sprintf("failed to parse %q annotaion: %s", annotationGracePeriod, err.Error()), ctx)
	...
	if err = result.CheckForIncompatibleFields(); err != nil {
		logEvent.ErrorInvalidConfiguration(reasonIncompatible, fmt.Sprintf("found incompatible fields: %s", err.Error()), ctx)
