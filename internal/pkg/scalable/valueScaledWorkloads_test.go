...
func TestValueScaledWorkload_ScaleDown(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantOriginalReplicas: values.StringReplicas(corev1.ServiceTypeLoadBalancer),
		...
		},
		{
			...
			initial:              "nginx",
			originalReplicas:     nil,
			wantOriginalReplicas: values.StringReplicas("nginx"),
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			summary, err := vsw.ScaleDown(values.AbsoluteReplicas(0))
...
func TestValueScaledWorkload_ScaleUp(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			originalReplicas: values.StringReplicas(corev1.ServiceTypeLoadBalancer),
		...
		},
		{
			...
			originalReplicas: values.StringReplicas("nginx"),
			wantRestored:     "nginx",
		},
		{
