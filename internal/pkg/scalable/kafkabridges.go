...
func getKafkaBridges(namespace string, clientsets *Clientsets, ctx context.Context) ([]Workload, error) {
	...
	list.SetGroupVersionKind(schema.GroupVersionKind{
		Group:   kafkaBridgeGVK.Group,
		Version: kafkaBridgeGVK.Version,
		Kind:    kafkaBridgeGVK.Kind + "List",
	})

	if err := clientsets.Client.List(ctx, list, ctrlclient.InNamespace(namespace)); err != nil {
		if apimeta.IsNoMatchError(err) {
			slog.Warn("strimzi CRD not found in cluster, skipping", "kind", kafkaBridgeGVK.Kind, "error", err)
			return nil, nil
		}

		return nil, fmt.Errorf("failed to get kafkabridges: %w", err)
	}
...
func parseKafkaBridgeFromBytes(rawObject []byte) (Workload, error) {
	...
	return &replicaScaledWorkload{&kafkaBridge{&u}}, nil
}
...
func (k *kafkaBridge) setReplicas(replicas int32) error {
...
func (k *kafkaBridge) getSavedResourcesRequests(_ int32) *metrics.SavedResources {
...
func (k *kafkaBridge) Compare(workloadCopy Workload) (jsondiff.Patch, error) {
	rswCopy, ok := workloadCopy.(*replicaScaledWorkload)
	if !ok {
		return nil, newExpectTypeGotTypeError((*replicaScaledWorkload)(nil), workloadCopy)
	}

	kCopy, ok := rswCopy.replicaScaledResource.(*kafkaBridge)
	if !ok {
		return nil, newExpectTypeGotTypeError((*kafkaBridge)(nil), rswCopy.replicaScaledResource)
	}
...
func (k *kafkaBridge) Update(clientsets *Clientsets, ctx context.Context) error {
