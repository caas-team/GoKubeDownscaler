...
For example `2000m` enables using up to
...
2. The CPU Limit doesn't guarantee that the Downscaler will use the full amount of CPU declared.
The pod can expand its
   CPU usage as long as the resources are available on the node at any given time.
If you need to guarantee a certain
