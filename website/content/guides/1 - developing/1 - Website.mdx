...
// TODO: tell ppl about: `repo`->link to repo, `repo:/path/to/file`, `repo:./path/to/file`, `repo:path/to/file`

Normally markdown links look like this: `[Some Text](https://example.com)`
To be able to easily update the links we decided to use global references for links to docs or guides.

- Links to external sites are still handled like this: `[some text](https://example.com)`
- Links to a heading on the same page: `[some text](#heading-id)`
- Links to the GoKubeDownscaler repository:
  - `[some text](repo:path/to/file)` or
    `[some text](repo:/path/to/file)` or
    `[some text](repo:./path/to/file)`
    -> `https://github.com/caas-team/GoKubeDownscaler/tree/main/path/to/file`
  - `[some text](repo)` -> `https://github.com/caas-team/GoKubeDownscaler`
...
Docusaurus caches most files and doesn't rebuild them until they themselves change (eg. rendered markdown files, [SVGO](https://svgo.dev/)/[SVGR](https://react-svgr.com/) optimized SVGs, etc.).
