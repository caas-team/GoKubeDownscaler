...
import { ExcalidrawWrapper } from "/src/components/Excalidraw.tsx";
...
Assets like PNGs, SVGs, [Excalidraw](#excalidraw-wrapper) or other files can be useful to have in documentation.

Assets should be placed in a `assets` folder in the folder in which the asset is being used.
...
For embedding `.excalidraw` files you need to use [our custom Excalidraw wrapper component](#excalidraw-wrapper).
...
The Excalidraw wrapper component is useful when you need to explain something more complex with a diagram.

The Excalidraw wrapper can render any excalidraw file.
...
<ExcalidrawWrapper data={exampleDiagram} className="w-1/3" />
```
