...
In most cases it's very simple to migrate and step 2 and 3 can be skipped.
...
The scaling [values](ref:docs-values) are now ordered into [layers](ref:docs-layers).
This makes it easier to understand where the values are taken from.

See the [breaking changes entry](#changed-value-hirarchy) for more information.
...
See the [features entry](#layers) for more information.
