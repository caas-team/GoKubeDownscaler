...
annotations.
By configuring the `annotationsCompliance` section, the administrator can ensure that regular users
...
- `onWorkloads.preventRemoval` ndicates whether to deploy a separate MutatingAdmissionPolicy to prevent the removal of downscaler
...
4. The user making the request does not match any of the authorized service accounts or users defined in the configuration
   (via regex patterns for namespaces/service accounts or specific users).

:::note
If the user has the permission to update the object, the removal of downscaler annotations is always permitted.
The next
paragraph explains how to prevent the removal of downscaler annotations.
:::

When `onWorkloads.preventRemoval` and/or `onNamespace.preventRemoval` is true, the following expression is evaluated
...
:::note
If the user has the permission to delete the entire resource (e.g., Namespace or Deployment), they can still delete the entire resource
even if they are not authorized to remove the downscaler annotations.
:::note
...
Users and administrators must ensure that workload templates comply with the required annotations.
Only an authorized user is permitted
to modify or remove these annotations.
Failure to do so may result in blocked updates or removal of the annotations.
For this reason,
we suggest to use a **MutatingAdmissionPolicy** or a **ValidatingAdmissionPolicy** without **Deny** action when possible.
