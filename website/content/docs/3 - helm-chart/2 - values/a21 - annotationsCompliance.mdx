...
description: >
  Learn how to enable annotationsCompliance in GoKubeDownscaler to prevent unauthorized creation,
  modification, or deletion of reserved downscaler annotations.
...
- `failurePolicy` controls what happens if a policy expression cannot be evaluated.
  It defaults to `Ignore` but it is recommended to set `Fail` to ensure that evaluation errors cannot bypass annotations compliance.
  Set it to `Ignore` only when admission availability is more important than enforcement.
...
- `authorizedGroupsRegex` array of regex strings that match groups that are authorized to set downscaler annotations.
- `onWorkloads.enabled` indicates whether the policy should be applied to workloads (Deployments, StatefulSets, etc.).
  At least one `includedResources` entry is required when workload compliance or removal prevention is enabled.
...
request.userInfo.username !=
  ...
  "{{ include \"go-kube-downscaler.webhookController.serviceAccountName\" . }}"
```
...
to prevent the removal of downscaler annotations from the configured resources.
In mutating mode, the mutator restores removed
annotations and the validating policy remains installed as a fail-closed fallback if mutation does not restore them.
