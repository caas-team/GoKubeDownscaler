...
if they were deployed by unauthorized users or service accounts.
If deployed, the policy is bound to every namespace.

## Validatingadmissionpolicy

The [validatingadmissionpolicy.yaml](repo:deployments/chart/templates/validatingadmissionpolicy.yaml) file creates a
ValidatingAdmissionPolicy that blocks objects on create/update with `downscaler/` annotations if these annotations were deployed by
unauthorized users or service accounts.
If deployed, the policy is bound to every namespace.
...
ValidatingAdmissionPolicyRemoval that blocks editing and removal of `downscaler/` annotations in namespaces
by unauthorized users or service accounts.
If deployed, the policy is bound to every namespace.
