...
Scopes are levels that hold behavioral and scaling configurations. These configurations can be divided into
two categories:
...
configuration is defined in multiple scopes. It can be visualized as pyramid, with the Default Scope at the bottom
and the Workload Scope at the top. Each scope can override the configurations of the scopes below it in the hierarchy.
...
[CLI Scope](#cli-scope) > [ENV Scope](#environment-scope) > [Default Scope](#default-scope). The following table shows
