...
INSERT HIERARCHICAL SCOPE IMAGE WITH CLI SCOPE HIGHLIGHTED

## Values

At CLI Scope, the following [values](ref:docs-values) can be configured using arguments to apply global
...
This will exclude every workload unless [more specific scopes](ref:docs-scopes) (e.g.: [namespace scope](ref:docs-namespace-scope),
...
At CLI Scope, the following [runtime configurations](ref:docs-runtime-configuration) can be configured using arugments
...
command, you can set the CLI Scope inside the args section of the `go-kube-downscaler` container in the deployment.
