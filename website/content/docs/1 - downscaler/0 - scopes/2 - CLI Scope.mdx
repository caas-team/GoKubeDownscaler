...
Setting a percentage value for `--downscale-replicas` when the `--include-resources` argument (described below) targets
resources that do not support percentage scaling (e.g. Deployments, StatefulSets, etc.) will result in a scaling error
when these resources are processed.
