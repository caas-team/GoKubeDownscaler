...
and [Namespace Scope](ref:docs-namespace-scope). It is the most specific scope available and its values
can't be overridden by other scopes.
...
Some annotations are incompatible with others. Before setting them, please check their compatibility clicking on the
hyperlink dedicated to each annotation.
