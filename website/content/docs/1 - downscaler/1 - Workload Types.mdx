...
Valid values for the workload type are the case-insensitive kubernetes resource names of the workloads. Workload types
to be included in the scaling process can be specified using the `--include-resources` argument at the
[CLI Scope](ref:docs-cli-scope#runtime-configuration)
