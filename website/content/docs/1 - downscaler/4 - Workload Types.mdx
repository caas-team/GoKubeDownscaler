...
For PodDisruptionBudgets the [downscale replicas](ref:docs-values#downscale-replicas) value can be an absolute number or a percentage.
If you are also scaling resources other than PodDisruptionBudgets the value set for
[downscale replicas](ref:docs-values#downscale-replicas) at Namespace Scope or CLI Scope must always be an absolute number or the
Downscaler will throw an error when processing other resources.
