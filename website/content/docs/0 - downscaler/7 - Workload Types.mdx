...
Valid values for the workload type are the case-insensitive kubernetes resource name of the Workload.
...
Scales by setting the node selector to match none of the nodes, which stops pods running on any node.
...
Scales by setting the paused replicas annotation to the [downscale replicas](ref:docs-values#downscale-replicas), which scales the workloads managed by the scaled object to the [downscale replicas](ref:docs-values#downscale-replicas).
