...
- Type: [Regex List](ref:docs-regex-list) (list of regex patterns matching labels)
  ...
  To match the workload having the label with any value you would use `example=.*`.
  :::
...
- Description: Makes the downscaler check the [grace period](ref:docs-values#grace-period) against the value of the annotation instead of the creation time of the workload.
