...
:::note
The IANA Time Zone database mainly supports regional/city timezones (example: `Europe/Berlin`, `America/Los_Angeles`)
instead of abbreviations (example: `CEST`, `PST`, `PDT`).
It supports some abbreviations like `CET`, `MET` and `PST8PDT` but these
(not including `UTC`) shouldn't be used, and only exist for backwards compatibility.
:::
