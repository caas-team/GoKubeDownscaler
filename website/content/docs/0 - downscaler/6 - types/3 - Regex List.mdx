...
For example if you want to match `my-namespace` and `namespace1` you cant just do a regex like `namespace` which then looks for the part `namespace` in the string.
What you would have to do is use the regex `.*namespace.*` to do so.

You can think of this like always having an start of string (`^`) and end of string (`$`) anchor around your expression.
