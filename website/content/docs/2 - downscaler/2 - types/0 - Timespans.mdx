...
description: Learn how to define timespans in GoKubeDownscaler to specify scaling schedules using time ranges, weekdays, and recurring patterns for Kubernetes workloads.
keywords:
  - timespans
  - absolute timespans
  - relative timespans
...
<script type="application/ld+json">
  {JSON.stringify({
    ...
    "@type": "TechArticle",
    "@id": "https://kube-downscaler.io/docs/timespans#techarticle",
    headline: "Timespans — GoKubeDownscaler",
    description:
      "Learn how to define timespans in GoKubeDownscaler to specify scaling schedules using time ranges, weekdays, and recurring patterns for Kubernetes workloads.",
    articleSection: "Types - Timespans",
    keywords: ["timespans", "schedules", "time ranges", "weekdays"],
    url: "https://kube-downscaler.io/docs/timespans",
    image: "https://kube-downscaler.io/img/social-preview.png",
    mainEntityOfPage: {
      "@type": "WebPage",
      "@id": "https://kube-downscaler.io/docs/timespans",
    },
...
There are four types of timespans:

- [Absolute timespans](#absolute-timespans): a timespan defined by two absolute points in time
- [Directional timespans](#directional-timespans): a timespan defined by a single absolute point in time,
  extending either forward (“from”) or backward (“until”)
- [Relative timespans](#relative-timespans): reoccurring on a weekly schedule
- [Boolean timespans](#boolean-timespans): statically always or never active
...
## Directional Timespans

- Format:
  `from <RFC3339-Timestamp>` or
  `from-<RFC3339-Timestamp>` or
  `until <RFC3339-Timestamp>` or
  `until-<RFC3339-Timestamp>` or
- Examples:

```text
from 2024-07-29T08:30:00Z
until 2024-07-29T16:00:00+02:00
from-2024-07-29T08:30:00Z
until-2024-07-29T16:00:00+02:00
```
...
Relative timespans can also be defined without a timezone, without a week frame or with both missing.
This setup
however requires a global timezone and/or week frame to be set, otherwise the timespan would not be valid and scaling
will result in an error.

- Format: `<Time-Of-Day-From>-<Time-Of-Day-To> <Timezone>`
- Requires: [DEFAULT_WEEKFRAME](ref:docs-values#weekframe) environment variable to be set
- Examples:
...
- Format: `<Weekday-From>-<Weekday-To> <Time-Of-Day-From>-<Time-Of-Day-To>`
- Requires: [DEFAULT_TIMEZONE](ref:docs-values#timezone) environment variable to be set
- Examples:

```text
    Mon-Fri 08:00-20:00                   # From Monday to Friday: from 08:00 to 20:00 in the timezone defined in the global timezone
    Sat-Sun 00:00-22:00                   # On the weekend: from 00:00 to 22:00 in the timezone defined in the global timezone
```

- Format: `<Time-Of-Day-From>-<Time-Of-Day-To>`
- Requires: [DEFAULT_WEEKFRAME](ref:docs-values#weekframe), [DEFAULT_TIMEZONE](ref:docs-values#timezone) environment variables to be set
- Examples:
...
Sometimes it's not enough to have just one timespan, in those cases you can define multiple.
...
Although you could mix [boolean timespans](#boolean-timespans) with the other ones,
