...
description: >
  Explore all runtime configuration options for GoKubeDownscaler, including CLI flags and environment
  variables to customize downscaling behavior in Kubernetes.
...
<script type="application/ld+json">
  {JSON.stringify({
    ...
    "@type": "TechArticle",
    "@id": "https://kube-downscaler.io/docs/runtime-configuration#techarticle",
    headline: "Runtime Configuration — GoKubeDownscaler",
    description:
      "Explore all runtime configuration options for GoKubeDownscaler, including CLI " +
      "flags and environment variables to customize downscaling behavior in Kubernetes.",
    articleSection: "Configurations - Runtime",
    keywords: ["runtime configuration", "flags", "cli", "options"],
    url: "https://kube-downscaler.io/docs/runtime-configuration",
    image: "https://kube-downscaler.io/img/social-preview.png",
    mainEntityOfPage: {
      "@type": "WebPage",
      "@id": "https://kube-downscaler.io/docs/runtime-configuration",
    },
...
- works for components: KubeDownscaler (you can still specify this argument inside the Webhook but types configured inside the
  MutatingWebhookConfiguration take precedence)
...
- Type: float64
- Description: Sets the maximum number of queries per second (QPS) that the downscaler can make to the Kubernetes API server.
  If you set QPS to a negative value rate limiting will be disabled even if you specify a Burst value.
  [Kubernetes Documentation](https://kubernetes.io/docs/reference/config-api/apiserver-eventratelimit.v1alpha1)
- Default: 500
- Where to set: [CLI Scope](ref:docs-cli-scope#runtime-configuration)
...
- Type: int
- Description: Sets the maximum burst rate the downscaler can use when throttling requests to the Kubernetes API server.
  Burst must be greater than 0, and it is only effective if QPS is set to a non-negative value.
  It is strongly recommended to set
  Burst to a value greater than QPS to allow for occasional spikes in request rates.
  [Kubernetes Documentation](https://kubernetes.io/docs/reference/config-api/apiserver-eventratelimit.v1alpha1)
- Default: 1000
- Where to set: [CLI Scope](ref:docs-cli-scope#runtime-configuration)
...
- Type: int (seconds)
- Description: Sets the timeout for requests to the Kubernetes API.
- Default: 30
- Where to set: [CLI Scope](ref:docs-cli-scope#runtime-configuration)

:::warning
If GoKubeDownscaler operations frequently time out, investigate API server health and admission webhook latency
before increasing the default value.
For example, an under-provisioned admission webhook may respond slowly, increasing the latency of API requests that invoke it,
including requests originating from GoKubeDownscaler and other cluster clients.
Timeouts can often be a symptom of broader cluster performance issues, increasing the timeout without
addressing the root cause may only mask the problem and lead to further issues.
...
Since, in that case, the downscaler will only run once, you might want to perform retries if such conflicts occur.
