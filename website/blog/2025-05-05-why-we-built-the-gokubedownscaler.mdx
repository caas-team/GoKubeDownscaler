...
This led us to decide that it was necessary to rewrite the project since doing a big refactoring would basically involve the same.
...
Go's concurrency model is also one of the easiest to work with.
This allowed us to build the downscaler
to use concurrent operations wherever possible making scans complete much faster.
...
We will be adding more linters and general tools to make sure the project's code stays clean and its dependencies up-to-date

## Conclusion

In summary, the lack of good first party apis for python, go's efficiency
